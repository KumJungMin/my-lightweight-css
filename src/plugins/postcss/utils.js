 ...
 */
export function getPostCssUtilities({ utilities, theme }) {
  ...
  for (const utilConfig of utilities) {
    ...
    if (tokenPath) {
      ...
      for (const ruleDef of rules) {
        ...
        const propsArray = Array.isArray(prop) ? prop : [prop];
        const valuesArray = Array.isArray(value) ? value.map(String) : null;
        const decls = [];

        if (valuesArray && valuesArray.length === propsArray.length) {
          // prop과 value 배열이 같은 길이면 zip 처리
          for (let i = 0; i < propsArray.length; i++) {
            decls.push(
              postcss.decl({ prop: propsArray[i], value: valuesArray[i] })
            );
          }
        } else {
          // 단일 value 또는 매핑 불가 시 모든 prop에 동일 value 적용
          const finalValue = valuesArray ? valuesArray.join(',') : String(value);
          for (const p of propsArray) {
            decls.push(
              postcss.decl({ prop: p, value: finalValue })
            );
          }
        }
        ...
        if (decls.length) {
          generatedRules.push(
            postcss.rule({ selector: selectorPattern }).append(...decls)
          );
        }
      }
    }
